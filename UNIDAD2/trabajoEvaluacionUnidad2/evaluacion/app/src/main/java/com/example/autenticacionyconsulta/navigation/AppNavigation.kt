...
import androidx.navigation.navArgument
import com.example.autenticacionyconsulta.Finales
...
fun AppNavigation(){
    ...
    NavHost(
        ...
        composable(route =AppScreens.Finales.route+"{modoEducativo}",
            arguments = listOf(navArgument(name = "modoEducativo"){
                type= NavType.StringType
            })){
            Finales(navController, it.arguments?.getString("modoEducativo"))
        }
        composable(route =AppScreens.Parciales.route){
            Parciales(navController)
        }
    }
}